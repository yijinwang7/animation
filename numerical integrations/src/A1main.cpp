...
void mouse_callback(GLFWwindow* window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT) {
        ...
        if (pressed) {
            xdown = xcurrent;
            ydown = ycurrent;
            findCloseParticles(xcurrent, ycurrent); //p1 is the closest particle to the mouse
            if (p1 != NULL && d1 < grabThresh) {
                wasPinned = p1->pinned;
                p1->pinned = false;
                grabbed = true;
                //create new particle at the location of the mouse
                mouse = particleSystem.createParticle(xcurrent, ycurrent, 0.0, 0.0);
                sp = particleSystem.createSpring(mouse, p1);
                sp->k = 1;
                sp->l0 = 0;
                //p1->p = glm::vec2(xcurrent, ycurrent);
                //p1->v = glm::vec2(0, 0);
        ...
        if (released) {
            if (!grabbed && !run) {
                ...
                p1->pinned = !wasPinned;
                particleSystem.removeSpring(p1, p1->springs[0]->p2);
                particleSystem.remove(p1);
                findCloseParticles(xcurrent, ycurrent);
            }
            grabbed = false;
...
int main(int argc, char** argv) {
    ...
    window = glfwCreateWindow(1280, 720, "COMP 559 W23 - A1 Particle System - Yijin Wang 260843756", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }
