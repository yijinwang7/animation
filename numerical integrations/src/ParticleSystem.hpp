...
public:
    ...
    void derivs(float t, VectorXf& p, VectorXf& dpdt) {
        ...
        if (useGravity) {
            for (Particle* p : particles) {
                //p->addForce(p->mass * glm::vec2(0, this->gravity));
                p->f.y = p->mass * gravity;
            }
        }

        //viscous air damping
        for (Particle* p : particles) {
            glm::vec2 air_damp = -viscousDamping * p->v;
            p->addForce(air_damp);
        }
        ...
        for (Particle* p : particles) {
            dpdt[index++] = p->v.x;
            dpdt[index++] = p->v.y;
            dpdt[index++] = (p->f.x)/(p->mass);
            dpdt[index++] = (p->f.y)/(p->mass);
        }
    ...
    float springDamping = .9;
    float viscousDamping = 0.6;
